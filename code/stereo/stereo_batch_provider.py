...
class PatchProvider(object):
    ...
    def __init__(self, data, patch_size=(7, 7), N=(4, 10), P=1):
        ...
        self._cache = 5
        self._lock = Lock()
        self._thread = None  # 初始化 _thread 为 None
    ...
    def iterate_batches(self, batch_size):
        ...
        patch = self.random_patch()  # 确保这个方法在类中定义了
        ...
        # start the thread
        if self._thread is None:  # 检查线程是否已经启动
            self._thread = Thread(
    ...
    def stop(self):
        self._stop = True
        if self._thread is not None:  # 确保 _thread 不为空
            self._thread.join()
            self._thread = None  # 重置 _thread 属性
